...
private:
  ...
  getFuncType(mlir::MLIRContext *context) {
    auto llvmI32Ty = mlir::IntegerType::get(context, 32);
    auto runType = mlir::LLVM::LLVMFunctionType::get(llvmI32Ty, {}, false);
    return runType;
  }
  ...
  getOrInsertPrintf(mlir::PatternRewriter &rewriter, mlir::ModuleOp module) {
    auto *context = module.getContext();
    if (module.lookupSymbol<mlir::LLVM::LLVMFuncOp>("test")) {
    ...
    rewriter.create<mlir::LLVM::LLVMFuncOp>(module.getLoc(), "test",
                                            runType);
    return mlir::SymbolRefAttr::get(context, "test");
  }
};
